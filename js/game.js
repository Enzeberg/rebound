...
// 	var center=new Vector(0.5*wallR*Math.cos(this.levelManager.level), 0.5*wallR*Math.sin(this.levelManager.level));
// 	this.enemy=new Enemy(center, this.ballRadius, 'red', this.cx);
// 	this.enemy.display();
// }

Game.prototype.prepareEnemy=function(){
	var d2origin=this.wall.radius/2, argument=this.levelManager.level;
	this.enemy=new Enemy(d2origin, argument, this.ballRadius, 'red', this.cx);
	this.enemy.display();
}

Game.prototype.moveBall=function(){
	var ball=this.ball, baffle=this.baffle, wall=this.wall, enemy=this.enemy;
	var self=this;
...
Game.prototype.stopRotatingBaffle=function(){ //event listener for 'stopRotatingBaffle' event
	var baffle=this.baffle;
	baffle.stopRotating();
