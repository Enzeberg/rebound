...
Ball.prototype.hitsBaffle=function(baffle){
	...
	var ball_angle=this.center.getPrincipalArgument();//球心与原点的连线与x轴正方向的夹角，范围：(-PI, PI].
	...
	if(this.current_d2baffle<this.last_d2baffle){//小球在接近挡板。若不加这条判断，会造成小球沿着挡板缓慢移动的怪异现象。
...
Ball.prototype.hitsWall=function(wall){
	...
	if(this.current_d2origin>this.last_d2origin){ //小球在远离原点，即在（从内向外）接近墙。若不加这条判断，会造成小球沿着墙缓慢移动的怪异现象。
...
Ball.prototype.rebound=function(theta){
	var velocityArg=this.velocity.getPrincipalArgument();//球的速度在以球心为原点的坐标系中的主辐角，范围：(-PI, PI].
